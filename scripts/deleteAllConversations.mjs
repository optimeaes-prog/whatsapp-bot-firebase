  ...
  try {
    ...
    for (let i = 0; i < batches.length; i++) {
      try {
        ...
        const batchDeleteCount = Math.min(batchSize, totalConversations - (i * batchSize));
        deletedCount += batchDeleteCount;
        console.log(`✓ Deleted batch ${i + 1}/${batches.length} (${batchDeleteCount} conversations)`);
      } catch (error) {
        errorCount += Math.min(batchSize, totalConversations - (i * batchSize));
